...
public:
	...
	void dibujar(Graphics^ g, Bitmap^ bmpPersonaje) {
		...
		g->DrawImage(bmpPersonaje, Aumento, PortionToUse, GraphicsUnit::Pixel);
		x += dx;
		y += dy;

	}
	void mover(Graphics^ g, Bitmap^ bmpPersonaje) {
		...
		case DIRECCIONES::Abajo:
			...
			if (IX >= 0 && IX < 3)
				...
				IX = 0;
			if (y + dy + (alto/14) >= g->VisibleClipBounds.Height - 60 || x+dx+ancho/14 >=632|| x < 70)
			{
				dx = 0;
				dy = 0;
			}
		...
		case DIRECCIONES::Derecha:
			...
			if (x  + dx+ancho/12 >= 632|| y + dy + (alto / 14) >= g->VisibleClipBounds.Height - 60|| y < 70)
			{
				dx = 0;
				dy = 0;
			}
		...
		case DIRECCIONES::Izquierda:
			...
			if (IX >= 0 && IX < 3)
				...
				IX = 1;
			if (x < 70|| y + dy + (alto / 14) >= g->VisibleClipBounds.Height - 60|| y < 70)
			{ 
				dx = 0;
			dy = 0;
		}
